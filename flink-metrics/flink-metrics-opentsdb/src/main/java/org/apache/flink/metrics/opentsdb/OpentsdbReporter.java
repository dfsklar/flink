...
/**
 * Largely based on the OpentsdbReporter class by ReadyTalk
 * https://github.com/ReadyTalk/metrics-opentsdb/blob/master/metrics3-opentsdb/src/main/java/com/readytalk/metrics/OpentsdbReporter.java
 *
...
//	public static final String ARG_CONVERSION_DURATION = "durationConversion";
	...
	// For testing purposes, allow the timestamp to be a static fake one so assertEquals can be used in tests.
	private Long fakeTimeStamp = null;

	public void setFakeTimestamp(long t) {
		this.fakeTimeStamp = t;
	}
...
//		this.durationFactor = 1.0 / TimeUnit.valueOf(conversionDuration).toNanos(1);
		...
		log.info("Configured OpentsdbReporter with {host:{}, port:{}}", host, port);
	}
	...
	/*
	 * From OpenTSDB documentation at http://opentsdb.net/docs/build/html/user_guide/writing.html,
	 * the send format is:
	 * put <metric> <timestamp> <value> <tagk1=tagv1[ tagk2=tagv2 ...tagkN=tagvN]>
	 *
	 * The emit-to-TSDB-via-socket technique was inspired
	 * by https://raw.githubusercontent.com/auxesis/collectd-opentsdb/master/org/collectd/java/OpenTSDB.java
	 */
	private void send(final String name, final String value) {
		try {
			...
			// Time
			long time = (this.fakeTimeStamp != null) ? this.fakeTimeStamp : System.currentTimeMillis() / 1000L;
            ...
            */
			...
			LOG.error("unable to send packet to opentsdb at '{}:{}'", address.getHostName(), address.getPort());
		}
	}
